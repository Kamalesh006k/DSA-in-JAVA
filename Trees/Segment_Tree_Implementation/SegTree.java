...
public class SegTree {
    ...
    void insert(int[] arr, int[] seg, int i,int low, int high){
        if(low == high){
            seg[i] = arr[low];
            return;
        }
        int mid = low +(high - low)/2;
        insert(arr, seg,2*i+1, low, mid);
        insert(arr, seg,2*i+2,mid+1, high);
    ...
    public static void main(String[] args){
        ...
        seg1.insert(arr, seg1.SegTree, 0, 0, n-1);
        ...
        SegTree seg2 = new SegTree(k);
        seg1.insert(arr, seg2.SegTree, 0, 0, n-1);
